...
import { questions, verticalQuestions, Question } from './questions';
...
export const TriviaVideo: React.FC<{ layout?: 'horizontal' | 'vertical' }> = ({ layout = 'horizontal' }) => {
    ...
    const activeQuestions = layout === 'vertical' ? verticalQuestions : questions;
    ...
    if (contentFrame < 0) {
        ...
        if (index >= 0 && index < activeQuestions.length) {
            ...
            // Outro logic (horizontal only usually)
            activeTheme = "theme-gold";
        }
    }

    // Calculate dynamic BGM volume for horizontal layout (Audio Ducking)
    // Intro frames: 0 to INTRO_DURATION
    // Outro frames start: INTRO_DURATION + (activeQuestions.length * QUESTION_DURATION)
            ...
            {layout === 'horizontal' && (
                <Sequence from={CHANNEL_INTRO_DURATION}>
                    ...
                    <Sequence from={0} durationInFrames={INTRO_DURATION}>
                        <Audio src={staticFile("intro_es.mp3")} volume={0.9} />
                    </Sequence>

                    {/* AI Outro Voice (20s) */}
                    <Sequence from={INTRO_DURATION + (activeQuestions.length * QUESTION_DURATION)}>
                        <Audio src={staticFile("outro_es.mp3")} volume={0.9} />
                    </Sequence>
                </Sequence>
            )}
